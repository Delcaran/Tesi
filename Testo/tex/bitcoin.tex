...
La serie di tutti blocchi calcolato forma una sequenza denominata \textbf{blockchain}.\\
...
\item
  Ogni nodo raccoglie le transazioni che riceve in un buffer denominato \emph{memory pool}.
\item
  Per ogni potenziale blocco, ogni nodo cerca di calcolare una proof-of-work.
\item
  Una volta trovata la prova il blocco è stato creato e viene inviato a tutti i nodi.
\item
...
Da questo si può dedurre un fatto importante che distingue la rete bitcoin dalle altre reti P2P: si può partecipare alla rete anche senza dover utilizzare in modo continuativo il relativo software, basta possedere un indirizzo Bitcoin.
...
\itemsep1pt\parskip0pt\parsep0pt
\item
  Un input è un riferimento ad un output di una precedente transazione   che contiene l'indirizzo di chi sta effettuando la transazione e la firma dimostrante la proprietà della moneta in tale output. Se ci   sono più input, l'importo degli output da loro referenziati viene   sommato ed il totale è il massimo valore utilizzabile dall'output.
\item
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{bitcoinpropagation_3.PNG}
\caption{Istogramma normalizzato del tempo che intercorre dal primo annuncio del blocco con una curva di interpolazione esponenziale.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{bitcoinpropagation_4.PNG}
\caption{Costo del ritardo per il 50°, 75° e 90° percentile. Il grafico è focalizzato sui valori più bassi delle ascisse per poter mostrare il comportamento costante presente dopo i 20kB.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{bitcoinpropagation_5.PNG}
